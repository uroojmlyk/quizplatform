...
import { useRouter } from 'next/navigation';
import Link from 'next/link';
import { Mail, Lock, Eye, EyeOff, ArrowRight, User, CheckCircle2, CheckCircle } from 'lucide-react';
...
function AuthInput({ label, icon: Icon, error, isFocused, isValid, rightSlot, ...props }: {
  ...
  const border = isFocused ? T.accentBorder : error ? 'rgba(239,68,68,0.3)' : isValid ? 'rgba(52,211,153,0.2)' : T.cardBorder;
  return (
    <div className="space-y-1.5">
      ...
      <div className="relative">
        <Icon className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none transition-colors duration-150"
          style={{ color: isFocused ? T.accent : error ? 'rgba(239,68,68,0.5)' : isValid ? T.accent : 'rgba(255,255,255,0.2)' }} />
        <input {...props}
          className="w-full pl-10 pr-10 py-3 rounded-xl text-sm outline-none transition-all duration-200 disabled:opacity-50"
          style={{
  ...
  return (
    <div className="hidden lg:flex lg:w-[44%] relative flex-col justify-between p-14 overflow-hidden">
      ...
      <div className="relative z-10 space-y-8">
        <div>
          ...
          <h2 className="leading-[1.08] tracking-[-0.03em] mb-4"
            ...
            Start teaching<br />
            <span style={{ color: 'rgba(255,255,255,0.55)' }}>smarter today.</span>
          </h2>
          <p className="text-sm leading-relaxed max-w-xs" style={{ color: T.textMuted }}>
            Build quizzes in minutes, assign to students, and watch results roll in live.
          </p>
        </div>
        <div className="space-y-3">
          {['Free forever — no credit card needed', 'AI quiz generator saves hours', 'Real-time scoring & analytics'].map((f) => (
      ...
      <div className="relative z-10 pl-4" style={{ borderLeft: '2px solid rgba(255,255,255,0.08)' }}>
        ...
        <p className="text-[10px] mt-2" style={{ color: 'rgba(255,255,255,0.12)' }}>— Malcolm X</p>
      </div>
    </div>
  );
}
        ...
        <div className="w-full max-w-[410px] relative z-10">
          ...
          {success && (
            <div className="mb-5 px-4 py-3.5 rounded-xl flex items-center gap-3"
              ...
              <CheckCircle className="w-4 h-4 shrink-0" style={{ color: T.accent }} />
              <div>
                <p className="text-sm font-semibold" style={{ color: T.accent }}>Account created!</p>
                <p className="text-xs" style={{ color: 'rgba(255,255,255,0.3)' }}>Redirecting you…</p>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <AuthInput label="Full Name" icon={User} type="text" value={form.name} placeholder="Your name"
              disabled={isLoading} isFocused={focusedField === 'name'} isValid={validation.name} error={errors.name}
              onChange={(e: any) => setForm(p => ({ ...p, name: e.target.value }))}
              onFocus={() => setFocusedField('name')} onBlur={() => setFocusedField(null)} />

            <AuthInput label="Email" icon={Mail} type="email" value={form.email} placeholder="you@example.com"
              disabled={isLoading} isFocused={focusedField === 'email'} isValid={validation.email} error={errors.email}
              onChange={(e: any) => setForm(p => ({ ...p, email: e.target.value }))}
              onFocus={() => setFocusedField('email')} onBlur={() => setFocusedField(null)} />
            ...
            <div className="space-y-2">
              <AuthInput label="Password" icon={Lock}
                type={showPassword ? 'text' : 'password'} value={form.password} placeholder="Min. 8 chars + number + symbol"
                disabled={isLoading} isFocused={focusedField === 'password'} isValid={validation.password} error={errors.password}
                rightSlot={EyeBtn(showPassword, () => setShowPassword(p => !p))}
                onChange={(e: any) => setForm(p => ({ ...p, password: e.target.value }))}
                onFocus={() => setFocusedField('password')} onBlur={() => setFocusedField(null)} />
              {strength && (
                <div className="space-y-1.5">
                  ...
                  <p className="text-[10px] pl-0.5 font-medium" style={{ color: strength.label }}>{strength.text} password</p>
                </div>
              )}
            </div>
            ...
            <AuthInput label="Confirm Password" icon={Lock}
              type={showConfirmPassword ? 'text' : 'password'} value={form.confirmPassword} placeholder="Repeat your password"
              disabled={isLoading} isFocused={focusedField === 'confirm'} isValid={validation.confirmPassword} error={errors.confirmPassword}
              rightSlot={EyeBtn(showConfirmPassword, () => setShowConfirmPassword(p => !p))}
              onChange={(e: any) => setForm(p => ({ ...p, confirmPassword: e.target.value }))}
              onFocus={() => setFocusedField('confirm')} onBlur={() => setFocusedField(null)} />
                ...
                {(['student', 'teacher'] as const).map(role => (
                  <button key={role} type="button" onClick={() => setForm(p => ({ ...p, role }))}
                    ...
                    style={form.role === role ? {
                      ...
                      color: T.accent, boxShadow: `0 0 16px ${T.accentGlow}`,
                    } : {
                      background: T.cardBg, border: `1px solid ${T.cardBorder}`, color: 'rgba(255,255,255,0.35)',
                    }}>
                    {role === 'student' ? '🎓' : '📚'} {role.charAt(0).toUpperCase() + role.slice(1)}
                  </button>
                ))}
            ...
            <div className="pt-1">
              <button type="submit" disabled={isLoading || success}
                ...
                style={{
                  background: isLoading || success ? T.accentBg : '#fff',
                  color: isLoading || success ? T.accent : '#080810',
                  boxShadow: isLoading || success ? 'none' : '0 4px 20px rgba(0,0,0,0.3)',
                  border: isLoading || success ? `1px solid ${T.accentBorder}` : '1px solid transparent',
                }}>
                {isLoading
                  ? <><div className="w-4 h-4 border-2 rounded-full animate-spin"
